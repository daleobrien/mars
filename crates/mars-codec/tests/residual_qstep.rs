...
use mars_codec::encode::{
    encode_image_rd, encode_image_with_options, EncodeOptions, EncodeParams, ResidualQuantisation,
};
...
fn qstep_closed_form_bounds_and_wire_rounding() {
    ...
    for lambda in [0.0, 0.01, 1.0, 50.0, 200.0, 800.0, 3200.0, f64::MAX] {
        ...
        let expected =
            (lambda.sqrt() * (6.0 / std::f64::consts::LN_2).sqrt()).clamp(1.0, 65535.0) as f32;
...
fn production_fixed_default_and_explicit_adaptive_roundtrip() {
    ...
    assert_eq!(
        EncodeOptions::default().residual_quantisation,
        ResidualQuantisation::default()
    );
    let source = image();
    for lambda in [None, Some(0.0), Some(50.0), Some(3200.0)] {
        ...
        let (fixed, fixed_leaves, _, _) = encode_image_with_options(
            &source,
            &p,
            &EncodeOptions {
        ...
        let (adaptive, adaptive_leaves, _, _) = encode_image_with_options(
            &source,
            &p,
            &EncodeOptions {
...
fn nonzero_residual_roundtrips_and_decoder_uses_step() {
    for step in [1.0, 8.0, 17.123456789, 65535.0] {
        ...
        assert_eq!(
            mars_format::write(&decoded_hdr, &decoded_leaves).unwrap(),
            bytes
        );
        let expected = ifs::decode_iterative(&hdr, &leaves, 10);
        assert_eq!(
            ifs::decode_iterative(&decoded_hdr, &decoded_leaves, 10),
            expected
        );
        ...
        for (i, end) in ends.into_iter().enumerate() {
            assert_eq!(
                progressive::decode(&progressive[..end]).unwrap().layers,
                i as u8 + 1
            );
            if i > 0 {
                assert_eq!(
                    progressive::decode(&progressive[..end - 1]).unwrap().layers,
                    i as u8
                );
            }
        }
...
fn color_planes_serialize_fixed_default_and_explicit_per_plane_lambda_steps() {
    ...
    for (subsampling, policy) in [Subsampling::Yuv444, Subsampling::Yuv420]
        .into_iter()
        .flat_map(|s| {
            [
                ResidualQuantisation::Fixed(ResidualQstep::LEGACY),
        ...
        })
    {
        ...
        assert_eq!(
            ...
            &[
                ...
                b'C',
                0,
                if subsampling == Subsampling::Yuv444 {
                    1
                } else {
                    2
                },
        ...
        for (i, source) in sources.iter().enumerate() {
            ...
            let (expected_hdr, expected_leaves, _, _) = encode_image_with_options(
                source,
                p,
                &EncodeOptions {
            ...
            let expected_step = match policy {
                ...
                ResidualQuantisation::LambdaAdaptive => {
                    ResidualQstep::from_lambda(p.lambda.unwrap())
                }
            };
        ...
        assert_eq!(
            color::decode_color_image_zoomed(&bytes, 10, 1.0).unwrap(),
            expected
        );
        let (progression, _) =
            color::decode_color_image_progression(&bytes, 10, None, 1.0, |_, _| {}).unwrap();
...
fn gray_color_wrapper_preserves_explicit_policy_and_fixed_default() {
    ...
    for policy in [
        ...
        assert_eq!(
            bytes,
            color::wrap_gray_stream(mars_format::write(&hdr, &leaves).unwrap())
        );
        assert_eq!(
            color::decode_color_image(&bytes, 10).unwrap().planes()[0],
...
fn malformed_qstep_headers_are_rejected() {
    ...
    for value in [
        ...
        assert_eq!(
            mars_format::read(&bytes),
            Err(MarsFormatError::InvalidResidualQstep)
        );
        let mut bytes = prog.clone();
        bytes[31..35].copy_from_slice(&value.to_le_bytes());
        assert_eq!(
            progressive::decode(&bytes).unwrap_err(),
            ProgressiveError::InvalidResidualQstep
        );
    }
    for end in 20..24 {
        assert_eq!(
            mars_format::read(&plane[..end]),
            Err(MarsFormatError::Truncated)
        );
    }
    for end in 31..35 {
        assert_eq!(
            progressive::decode(&prog[..end]).unwrap_err(),
            ProgressiveError::Truncated
        );
    }
    let mut plane = plane;
    plane[4] = 2;
    assert_eq!(
        mars_format::read(&plane),
        Err(MarsFormatError::UnsupportedVersion(2))
    );
    let mut prog = prog;
    prog[4] = 2;
    assert_eq!(
        progressive::decode(&prog).unwrap_err(),
        ProgressiveError::UnsupportedVersion(2)
    );
}
