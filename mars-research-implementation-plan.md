...
### Step 14 — P5c: several retrieved candidates in RD mode competition

- Added `EncodeOptions::rd_candidates` (default 1) and `Ctx::rd_candidates`, threaded from the CLI and the experiment schema into `walk_rd`, which now asks the provider for a ranked list instead of a single winner. The exhaustive scan moved into `search_top_k(request, max)`: it takes the same `SearchRequest` the `SearchProvider` trait already carries (which is also what keeps it inside the workspace's seven-argument lint), and retains the best `max` by ascending RMS with a strict `<`, so `max == 1` reproduces the historical `best.is_none_or(|b| rms < b.rms)` winner exactly — including which of two tied candidates wins — and reports the same `evals`.
- The provider hook is a defaulted `SearchProvider::search_candidates` returning only the single winner, overridden by `ExhaustiveSearch`; `best_mode_leaf` iterates the ranked list for modes 2/3 (unchanged for modes 0/1), so a candidate with slightly worse RMS but cheaper coordinate/coefficient symbols or a better residual fit can win `J`. No stream syntax changed: the winner's fields were already priced by `leaf_events`, and the scan itself is the same scan, so `search_evals` is identical between arms and the added cost is per-leaf mode pricing only. `encmars --rd-candidates` requires the RD path and the exhaustive search, and the experiment schema enforces the same, so the option cannot silently degrade to one candidate under `--method fisher`/`random`/`apcc`.
- `ColorEncodeParams::rd_candidates` exists so the ordinary (no-`--method`) grayscale/colour path does not drop the option: an earlier cut of this step passed only the residual policy into `encode_color_image_with_residual_quantisation`, and the first measurement therefore compared two identical arms (byte-identical streams) — caught by reading the runner's recorded `encode_args` against the codec's actual behaviour rather than assuming the flag took effect. That first grid was discarded, not reported.
- Tests: top-k search semantics (`max == 1` reproduces `search_with_shift`'s winner, ascending RMS, same evals, distinct fitted positions); the aggregate internal `J` is provably ≤ the `max == 1` `J` under the identical frozen snapshot (the same bottom-up induction as the mode-mask test); `rd_candidates: 1` is byte-identical to `EncodeOptions::default()` and a wide list is a no-op when only mode 0 competes; a colour round trip with `rd_candidates: 3` proves the field reaches every plane; CLI/`Arm` validation tests; `marsbench experiment` passes `--rd-candidates` only when it differs from 1.
- **Measurement** (`configs/research-p5c.json`, artifacts `target/p5c/`; kodim01+02 grayscale, modes 0/2/3, fixed residual step 8, no search-method override, threads 4, ~19 min):

| image | λ | k=1 bpp | k=1 PSNR-Y | k=8 bpp | k=8 PSNR-Y | Δ bits | Δ PSNR-Y | encode s (1→8) |
|---|---:|---:|---:|---:|---:|---:|---:|---:|
| kodim01 | 200 | 0.5393 | 26.5206 | 0.5598 | 26.6388 | +3.80% | +0.118 dB | 91.2 → 102.2 |
| kodim01 | 800 | 0.1678 | 23.1354 | 0.1733 | 23.1667 | +3.30% | +0.031 dB | 90.9 → 102.3 |
| kodim01 | 3200 | 0.0861 | 21.5172 | 0.0938 | 21.6461 | +8.98% | +0.129 dB | 90.9 → 102.3 |
| kodim02 | 200 | 0.1417 | 30.6915 | 0.1525 | 30.8006 | +7.64% | +0.109 dB | 67.3 → 78.6 |
| kodim02 | 800 | 0.0671 | 29.0229 | 0.0756 | 29.1349 | +12.74% | +0.112 dB | 67.2 → 79.6 |
| kodim02 | 3200 | 0.0409 | 28.1497 | 0.0469 | 28.2825 | +14.51% | +0.133 dB | 66.8 → 78.5 |

- **Every point moves up and to the right, and the quality gain does not scale with the rate it costs.** At kodim01 λ=200 the trade is roughly break-even against the codec's own local slope (+3.8% bits for +0.118 dB); by kodim02 λ=3200 it is clearly unfavourable (+14.5% bits for +0.133 dB). Encode time rises 11–18%. Widening the candidate list buys quality only by buying rate, and it buys it at a worsening exchange rate as λ grows.
- **Decision: not promoted.** P5c's proposed target (≥3% mean BD-rate improvement at ≤25% encode cost) is not met, and the sign is wrong in the direction that matters — this is a rate cost for a smaller quality gain, not a rate saving. At this step the grid had only three λ points, so §11.4's four-point rule was unmet and no BD-rate interval was claimed; step 17 later completed the grid and measured one. The option stays in the tree, default-off and byte-identical at 1, as the control P5b/P6 can be measured against.

### Step 15 — test-suite runtime (dev-profile optimization and frame-size scoping)

- The plan's own §14 command `cargo test --locked -p mars-codec` was unusable. The dev/test profile defaults to `opt-level = 0`, and the exhaustive fractal search is 20–50x slower there than in release: `color_gate`'s full-frame colour round trip alone measured 1393s wall / 13480s CPU, and the whole workspace suite ran ~30–45 minutes. The Justfile's gate recipes already sidestep this by passing `--release`; the routine `cargo test` paths did not.
- Added `[profile.dev] opt-level = 2` to the workspace manifest (the `test` profile inherits it), keeping dev-profile semantics — debug assertions and debuginfo stay on — and deliberately not 3, since this profile is also for debugging. Measured 8–20x on the numeric paths: a synthetic 96x80 4:4:4 encode 482ms → 58ms, and `quality()` on a full 768x512 frame 1.08s → 57ms (SSIM 444ms → 22ms, MS-SSIM 588ms → 29ms). No fast-math is enabled, so no floating-point result changes; the pinned byte-exactness tests still pass unchanged.
- Three tests then encoded full frames where the property under test does not depend on frame size, so each was scoped to a documented crop: `color_gate`'s real-Kodak round trip (768x512 → 256x256), `mars-codec`'s `parallel_determinism` (the committed 512x512 Mandelbrot → 256x256; it still exercises the parallel and sequential-cutoff branches, and `mars-search`'s equivalent test is already 2.5s because its `Fisher` retriever restricts candidates), and `cli_a_gate`'s default-behaviour check (768x512 → a 128x128 crop built from the corpus plane in the test's scratch dir).
- Result: `color_gate` 90.6s → 2.5s, `mars-codec::parallel_determinism` 56.9s → 2.6s, `cli_a_gate` 183.6s → 0.4s, and `cargo test --locked --offline --workspace` now completes in **77s** wall with every test passing. The slowest remaining binaries are the corpus-dependent gates (`learned_gate` 27.5s, `funnel_gate` 16.8s, `classical_methods_gate` 10.5s), which is an order of magnitude below what was removed here.
- Two things found here, outside this step's scope, both since fixed: (1) `cli_a_gate`'s `omitting_adaptive_density_matches_explicit_false_byte_for_byte` compares two `encmars` invocations that both *omit* `--adaptive-density` — a boolean flag has no explicit-false spelling — so the comparison is vacuous and would pass even if the flag's default changed; its docstring's stated claim ("omitting the flag must still produce exactly what it always has") would need a pinned stream hash to be meaningful. **Fixed in step 16.** (2) The local clippy (1.97) flags five pre-existing lints in code this step did not author (`mars-entropy` `manual_range_contains`, `search_provider` `type_complexity`, `postprocess` `unnecessary_cast`, `encmars` `items_after_test_module`, `train_learned` `unusual_byte_groupings`); `cargo clippy --workspace --all-targets -- -D warnings` therefore already failed before this step. **All five are now fixed** — the range predicate became `(-1e-9..128.0).contains(&slack)`, the recorded-request type got a `RecordedRequests` alias, the redundant cast was dropped, `encmars`'s test module was moved after the items it preceded (verified a pure permutation of the file's lines), and the seed literal was regrouped — so `cargo clippy --workspace --all-targets -- -D warnings` now passes clean.

### Step 16 — make `cli_a_gate`'s default-flag check non-vacuous

- Step 15 flagged `cli_a_gate`'s `omitting_adaptive_density_matches_explicit_false_byte_for_byte` as vacuous. Rewritten as `omitting_adaptive_density_reproduces_the_pinned_default_stream`: the omitted-flag stream is now pinned by SHA-256 (the convention `mars-codec/tests/search_provider.rs::pinned_pre_provider_streams` already uses), and the check also asserts the flag is *observable* on its input, so the pin cannot be trivially satisfied.
- The observability assertion is load-bearing, not decoration. Measured directly at the check's own arguments: kodim01's 128x128 top-left encodes **byte-identically** with and without `--adaptive-density` (`33c59d23…`, 1208 bytes both ways). Post-D48 the flag only reroutes a low-RMS block onto a doubled domain stride, and natural imagery rarely qualifies — so a corpus crop cannot detect a default flip even if the two arms had differed. The check now encodes a synthetic gentle gradient instead, where every block routes through `adaptive_shift`: verified observable, off `55ccaacf…` vs on `5af49813…` (same 76-byte length), 9,914,880 vs 2,644,480 evals.
- Dropping the corpus crop also makes the check hermetic, so unlike the opt-in sweep beside it this now runs in CI's quick job, which never fetches the corpus and previously skipped it entirely.
- Validation: rewritten test passes (0.19s); `cargo clippy -p mars-cli --all-targets` clean apart from the pre-existing `encmars` `items_after_test_module`; changed file rustfmt-clean.

### Step 17 — P5c's four-point BD-rate (λ=50 completion)

- Completed P5c's λ grid with λ=50, as the companion config `configs/research-p5c-lambda50.json` (arms `k1-lambda50`/`k8-lambda50`, same codec/decoder/groups, run with `--out-dir target/p5c`). Adding λ=50 to `research-p5c.json` instead would have been wasted compute: the runner resumes by `experiment_id` and `case_id` hashes `config_sha256`, so editing the main config invalidates and recomputes all twelve existing cases under an unchanged encoder. The companion's λ set is disjoint, so the combined store holds exactly four distinct points per curve per image, and the new rows' `encoder_binary.sha256` (`c3be8c15…`) and `decoder_binary.sha256` (`fbb440ba…`) equal the old rows' — the four-point curve is provably one binary. Four cases, four successes, no failures.

| image | λ | k=1 bpp | k=1 PSNR-Y | k=8 bpp | k=8 PSNR-Y | Δ bits | Δ PSNR-Y | encode s (1→8) |
|---|---:|---:|---:|---:|---:|---:|---:|---:|
| kodim01 | 50 | 1.2076 | 30.2773 | 1.2201 | 30.3289 | +1.03% | +0.052 dB | 91.6 → 104.9 |
| kodim02 | 50 | 0.3784 | 33.5328 | 0.3971 | 33.6566 | +4.94% | +0.124 dB | 67.4 → 79.8 |

- **Decision: not promoted, now with a BD-rate rather than a diagnostic.** `marsbench experiment-report --store target/p5c/results.jsonl --reference-group candidates1` reports candidates8 vs candidates1 at **+0.98% on kodim01 and +3.45% on kodim02, mean +2.21%** — positive meaning more rate for equal PSNR-Y, i.e. worse. That is the opposite sign to P5c's ≥3% improvement target, so the reject stands on the metric it was originally meant to be judged by. λ=50 is the mildest point (kodim01 +1.03% bits for +0.052 dB, kodim02 +4.94% for +0.124 dB) and still does not rescue the curve.

### Step 18 — P5b feasibility: no useful admissible bound on the frozen objective

- P5b was gated on an *admissible* lower bound existing before a pruner was built. Checked against the walk and the objective directly; the answer is no, and the reason is structural rather than a matter of a cleverer bound.
- A node's value is `V = min(leaf_J, Σ_children V + λ·split_flag(1))` over a frozen, additive snapshot, so a sound bound is well-defined in principle, and pruning the split branch needs `LB(children) ≥ leaf_J`. The only cheap sound ingredients are `D ≥ 0` and `R ≥ R_floor(size_class)`, the latter being the sum over a leaf's required fields of the per-context *minimum-over-symbols* priced bits. `bits_for` is strictly positive by construction (`AdaptiveModel::new` seeds every count at 1, so no symbol is ever free), so a positive floor exists and is computable.
- **Distortion has no cheap sound lower bound, and it dominates `J`.** Modes 2/3 (fractal, fractal+residual) can drive `D` toward zero, so block variance, edge energy, and even mode 1's own affine-fit error are *not* lower bounds on the minimum over the enabled modes; using any of them would be inadmissible and could change leaves/bytes, violating P5b's own gate. Only `D ≥ 0` survives.
- With `D ≥ 0`, the resulting condition is content-independent: prune the split when `leaf_J ≤ λ·(split_flag(1) + 4·(R_floor(half) + flag))`. Because each field takes its cheapest symbol *independently*, `4·R_floor` sits roughly an order of magnitude below a realistic child `J` (a real child pays distortion plus non-minimal symbols), so it fires only for near-constant blocks — negligible on natural imagery.
- The frustration is that the payoff *would* be large if a bound existed: a 768x512 min_size-4 image presents ≈7,700 split-vs-leaf decisions, and the chosen partition's split nodes are only a small fraction of them, so the great majority of decision nodes keep the leaf and could in principle be cut without recursing.
- **Verdict: do not build P5b as specified.** Keep the full walk and take the plan's own fallback (caching / work reduction), or pursue the audit-bias re-snapshot (D41 measured +3.90%). If empirical certainty is wanted before closing it, a ~30-line no-behaviour-change instrumentation that counts the prune condition's hits would confirm the expected ≈0% rate.
...
**Research status:** P0 is partially implemented, not complete. Residual-step metadata, focused round-trip tests, and a three-arm serialized-stream experiment exist. A restricted file-to-file smoke and shared production retrieval interface are implemented (steps 6/7); random (step 8) and APCC (step 10) are opt-in providers; the out-of-loop boundary filter is implemented (step 11); the strict config-driven experiment runner is implemented (step 12); P5a's objective audit is implemented and measured (step 13), and it found the surrogate's error to be localized in the partition and mode fields rather than diffuse. P5c's wider RD candidate competition is implemented and measured (step 14) and was **not** promoted: it bought quality only by buying rate, at 11–18% more encode time. Step 15 optimized the dev/test profile and scoped three oversized tests to crops, taking `cargo test --workspace` from ~30–45 minutes to 77s. Step 16 made `cli_a_gate`'s default-flag check non-vacuous (a pinned stream hash on an input where the flag is actually observable); step 17 completed P5c's λ grid with λ=50 and produced a four-point BD-rate (**+2.21% mean** for the wider candidate list — a rate cost, so still not promoted); and step 18 checked P5b's premise and found no useful admissible bound on the frozen objective, closing P5b as specified. The sparse multi-domain format remains proposed. P5 extends an existing RD optimizer; it is not a new optimizer implementation.
...
**Feasibility checked in execution step 18: no useful admissible bound exists, so the gate's own escape clause applies — keep full traversal and improve caching instead.** A sound lower bound on an unsearched subtree is limited to `D ≥ 0` plus a per-size-class rate floor built from each field's cheapest symbol, because fractal and residual modes can drive distortion toward zero, making variance, flatness, edge energy, and mode 1's own affine error inadmissible as bounds on the minimum over enabled modes. That rate-only floor is content-independent and sits about an order of magnitude below a realistic child `J`, so the pruning condition fires only for near-constant blocks. The payoff would be large if a bound existed (the chosen partition's split nodes are a small fraction of the ≈7,700 decisions a 768x512 min_size-4 image presents), but no sound bound captures the distortion term that decides those comparisons.

### P5c: deliberate quality extensions

**Implemented and measured in execution steps 14 and 17.** `encmars --rd-candidates N` (and the experiment schema's `rd_candidates`) offer the `N` best-RMS retrieved candidates to modes 2/3 instead of only the minimum-RMS winner; `N = 1` is byte-identical to the historical behaviour, and only the exhaustive provider can rank candidates, so a candidate-restricted method is refused rather than silently narrowed. On kodim01/02, modes 0/2/3, λ ∈ {50, 200, 800, 3200}, `N = 8` raised bits 1.0–14.5% for only 0.03–0.13 dB PSNR-Y and cost 11–18% more encode time — every point traded rate for quality, at a worsening exchange rate as λ grew. Completion at λ=50 (step 17) yields a proper four-point BD-rate, **+0.98% (kodim01) and +3.45% (kodim02), mean +2.21%** for the wider list — a rate cost at equal PSNR-Y, the opposite of the ≥3% improvement target. Retained default-off as a control, not promoted.
...
**Next deliverable:** P5a (step 13), P5c (steps 14 and 17) and the P5b feasibility check (step 18) are all delivered. P5c was **not** promoted: its completed four-point BD-rate is **+2.21% mean** (worse), so the wider candidate list buys quality only by buying rate at 11–18% more encode time. P5b is closed as specified — the only sound lower bound on an unsearched subtree (`D ≥ 0` plus an optimistic price-floor rate term) is far too loose to prune, and distortion, the term that actually decides the comparison, admits no cheap sound bound because fractal and residual modes can drive it to zero. Remaining P5 work is therefore the audit's own bias (a re-snapshotting objective, which D41 already measured once as a +3.90% regression) or moving on to P4's retrieval challengers / P6's in-loop smoothing. Either way, do not register a new search enum before measured curves exist.
