...
- **Decision: not promoted.** P5c's proposed target (≥3% mean BD-rate improvement at ≤25% encode cost) is not met, and the sign is wrong in the direction that matters — this is a rate cost for a smaller quality gain, not a rate saving. No BD-rate interval is claimed: the grid has three λ points and §11.4 requires four, so this is a per-point diagnostic rather than a curve comparison, and λ=50 is unmeasured. The option stays in the tree, default-off and byte-identical at 1, as the control P5b/P6 can be measured against.
...
- Two things found and **not** changed, since neither is this step's scope: (1) `cli_a_gate`'s `omitting_adaptive_density_matches_explicit_false_byte_for_byte` compares two `encmars` invocations that both *omit* `--adaptive-density` — a boolean flag has no explicit-false spelling — so the comparison is vacuous and would pass even if the flag's default changed; its docstring's stated claim ("omitting the flag must still produce exactly what it always has") would need a pinned stream hash to be meaningful. (2) The local clippy (1.97) flags five pre-existing lints in code this step did not author (`mars-entropy` `manual_range_contains`, `search_provider` `type_complexity`, `postprocess` `unnecessary_cast`, `encmars` `items_after_test_module`, `train_learned` `unusual_byte_groupings`); `cargo clippy --workspace --all-targets -- -D warnings` therefore already failed before this step, and the step's own crates are clean once those five are allowed.
...
**Implemented and measured in execution step 14.** `encmars --rd-candidates N` (and the experiment schema's `rd_candidates`) offer the `N` best-RMS retrieved candidates to modes 2/3 instead of only the minimum-RMS winner; `N = 1` is byte-identical to the historical behaviour, and only the exhaustive provider can rank candidates, so a candidate-restricted method is refused rather than silently narrowed. On kodim01/02, modes 0/2/3, λ ∈ {200, 800, 3200}, `N = 8` raised bits 3.3–14.5% for only 0.03–0.13 dB PSNR-Y and cost 11–18% more encode time — every point traded rate for quality, at a worsening exchange rate as λ grew. Retained default-off as a control, not promoted.
...
**Next deliverable:** P5a's audit (step 13) and P5c's wider candidate competition (step 14) are both delivered and measured. P5c was **not** promoted: it bought quality only by buying rate, at 11–18% more encode time. P5b (an admissible branch-and-bound bound on the frozen objective; gate: identical objective, leaves, bytes and decoded pixels versus the unpruned walk) is worth attempting only if such a bound actually exists — variance/flatness alone does not bound what a fractal or residual mode can achieve. Otherwise the remaining P5 work is the audit's own bias (a re-snapshotting objective, which D41 already measured once as a +3.90% regression) or moving on to P4's retrieval challengers / P6's in-loop smoothing. Either way, do not register a new search enum before measured curves exist.
